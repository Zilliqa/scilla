...
module Args = struct
  ...
  let mk index (tr : transition) ~(contract : contract) ~group ~env =
    ...
    let mk_path name def fallback =
      ...
      value file_path ~default:(dir ^/ fallback)
    in
    let init = mk_path tr.init contract.init "init.json" in
    let state = mk_path tr.state contract.state "state.json" in
    let blockchain =
      mk_path tr.blockchain contract.blockchain "blockchain.json"
    in
    let message =
      tr.message |> Option.value ~default:(tr.name ^. "json") |> fun f ->
      dir ^/ "transitions" ^/ f
    in
  ...
  let extra_args =
    ...
    | StateService.IPC sock_addr ->
        if contract.ipc then
          ...
          let balance =
            StateIPCTest.setup_and_initialize ~start_mock_server:false
              ~sock_addr ~state_json_path:args.state
          in
