...
let deploy_library args gas_remaining =
  let parse_lmodule =
    FrontEndParser.parse_file ScillaParser.Incremental.lmodule args.input in
...
let run (args : args) : Yojson.Basic.t =
  ...
  let parse_module =
    FrontEndParser.parse_file ScillaParser.Incremental.cmodule args.input in
  ...
  | Ok cmod ->
      ...
      then
        ...
        let (cstate', remaining_gas', field_vals) =
          check_extract_cstate args.input init_res gas_remaining in
        ...
        if is_ipc then
          ...
          let cstate, gas_remaining', _ = check_extract_cstate args.input init_res gas_remaining in
          ...
          (* Prints stats after the initialization and returns the initial state *)
          (* Will throw an exception if unsuccessful. *)
          let cstate, gas_remaining', field_vals = check_extract_cstate args.input init_res gas_remaining in
