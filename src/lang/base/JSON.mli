...
(** Message json parsing and printing. A message json has three mandatory fields a [_tag] specifying
    the transition to be invoked, an [_amount] field, specifying amount to be transferred,
    and a [_sender] field to convey the sender of the message. These two are followed by an array of json objects for parameters to the transition. These parameters
    are encoded similar to the array of variables in [ ContractState ]. It is expected that the types of parameters match with those in the contract
    definition.
...
(** Parses and returns a list of [(pname,pval)], with
  [_tag], [_sender] and [_amount] at the beginning of this list. Invalid inputs in the json are ignored *)
