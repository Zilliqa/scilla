...
(*            let _ = printf "Declared adts: %d, Declared constructors: %d\n" (List.length adts_in_scope) (List.length adt_ctrs_in_scope) in
              let _ = printf "Analyzing entry %s\n" (match entry with | LibVar (s, _) | LibTyp (s, _) -> (get_id s)) in *)
            ...
            (* LibTyp *)
            | Some adt ->
                pure @@ (new_entry :: rec_entries, adt :: datatypes, adt.tname :: adts_in_scope, (List.map adt.tconstr ~f:(fun ctr -> ctr.cname)) @ adt_ctrs_in_scope ))
        ~init:([], [], [], [])
        lentries in
    pure @@ (
  ...
  let recursion_module
    ...
    let _ = List.iter elibs_adts ~f:DataTypeDictionary.add_adt in
