...
module ScillaDisambiguation (SR : Rep) (ER : Rep) = struct
  ...
  type nsopt_name_adr_dict = (string option, (string, string) List.Assoc.t) List.Assoc.t
  ...
  let add_key_and_lib_address_to_dict (dict : nsopt_name_adr_dict) ns_key name_key value =
    let old_nm_dict =
      Option.value (List.Assoc.find dict ns_key ~equal:[%equal : String.t option])
        ~default:[] in
    let new_nm_dict = List.Assoc.add old_nm_dict name_key value ~equal:String.(=) in
  ...
  let remove_local_id_from_dict (dict : nsopt_name_adr_dict) var =
    ...
    let old_nm_dict =
      Option.value (List.Assoc.find dict None ~equal:[%equal : String.t option])
      ~default:[] in  
  ...
  let check_duplicate_dict_entry (dict : nsopt_name_adr_dict) ns_key name_key msg =
  ...
  let rec disambiguate_type typ_dict t =
  ...
  let disambiguate_literal (dicts : name_dicts) l =
    ...
    let rec recurser l =
      ...
      | Map ((kt, vt), mentries) ->
          ...
          let%bind res_kt =
            disambiguate_type dicts.typ_dict kt
          in
          let%bind res_vt =
            disambiguate_type dicts.typ_dict vt
          in
          let flat_table =
    ...
    let folder (var_dict_acc, dis_stmts_acc_rev) srep =
      ...
      let%bind dis_s, new_var_dict =
        ...
        | Bind (x, e') ->
            ...
            let%bind dis_e' =
              disambiguate_exp
                { dicts with var_dict = var_dict_acc }
                e'
            in
            (* x is now in scope as a local, so remove from var dictionary *)
            let new_var_dict =
        ...
        | MatchStmt (x, pss) ->
            ...
            let%bind dis_pss =
              mapM pss ~f:(fun (p, ss) ->
                  ...
                  let ss_var_dict =
                    List.fold bounds ~init:var_dict_acc
                      ~f:(fun var_dict' x ->
                        remove_local_id_from_dict var_dict' (as_string x))
                  in
                  let%bind dis_ss =
                    disambiguate_stmts
                      { dicts with var_dict = ss_var_dict }
                      ss
                  in
                  pure (dis_p, dis_ss))
            in
  ...
  let disambiguate_component (dicts : name_dicts) comp =
    ...
    let body_var_dict =
      List.fold_left comp_params ~init:dicts.var_dict
  ...
  let disambiguate_lib_entry (dicts : name_dicts) libentry this_address =
    ...
    | LibVar (x, topt, e) ->
        ...
        let%bind dis_topt =
          option_mapM topt
            ~f:(disambiguate_type dicts.typ_dict)
        in
        ...
        let res_dicts =
          {
            dicts with
            var_dict =
              add_key_and_lib_address_to_dict dicts.var_dict None (as_string x) this_address
          }
        in
    ...
    | LibTyp (tname, ctrs) ->
        ...
        let%bind res_typ_dict =
          ...
          in
          let%bind () = check_duplicate_dict_entry dicts.typ_dict None (as_string tname) msg in
          pure @@
        ...
        let%bind res_ctr_dict =
          ...
          foldM ctrs ~init:dicts.ctr_dict ~f:(fun ctr_dict_acc (ctr : ctr_def) ->
              ...
              let%bind () = check_duplicate_dict_entry ctr_dict_acc None (as_string ctr_name) msg in
              pure @@
        ...
        let res_dicts =
          {
            dicts with
            typ_dict = res_typ_dict;
            ctr_dict = res_ctr_dict;
          }
        in
  ...
  let build_import_dicts imports extlibs init_extlibs_map =
    ...
    foldM imports ~init:([], [], [], [])
      ~f:(fun ( ns_dict_acc,
        ...
        let lib_address = find_lib_address lib in
        match ns_opt with
        | Some ns ->
            ...
            let ns_dict =
              add_key_and_lib_address_to_dict ns_dict_acc (as_string ns) lib_address
            in
            pure
              (ns_dict, var_dict_acc, typ_dict_acc, ctr_dict_acc)
        | None ->
            (* No namespace defined - add all simple names *)
            let open PostDisSyntax in
            let open PostDisIdentifier in
            let%bind var_dict, typ_dict, ctr_dict =
              foldM lib.lentries
                ~init:(var_dict_acc, typ_dict_acc, ctr_dict_acc)
                ~f:(fun ( var_dict_acc',
                          typ_dict_acc',
                          ctr_dict_acc' )
                        lentry
                  ...
                  | LibVar (x, _, _) ->
                      ...
                      in
                      let%bind () =
                        check_duplicate_dict_entry var_dict_acc unqualified_x msg
                      in
                      (* Add unqualified_x -> lib_address to var dictionary *)
                      let var_dict =
                        add_key_and_lib_address_to_dict var_dict_acc'
                          unqualified_x lib_address
                      in
                      pure
                        (var_dict, typ_dict_acc', ctr_dict_acc')
                  | LibTyp (tname, ctr_defs) ->
                      ...
                      let typ_dict =
                        add_key_and_lib_address_to_dict typ_dict_acc'
                      ...
                      pure (var_dict_acc', typ_dict, ctr_dict))
            in
            (* All library definiens added *)
            pure (ns_dict_acc, var_dict, typ_dict, ctr_dict))
  ...
  let disambiguate_cmodule cmod (extlibs : PostDisSyntax.libtree list) init_extlibs_map this_address =
    ...
    let%bind ns_dict, imp_var_dict, imp_typ_dict, imp_ctr_dict =
    ...
    in
    let contract_typ_dict =
      List.fold_left lib_typs ~init:imp_typ_dict
        ~f:remove_local_id_from_dict
    in
    let contract_ctr_dict =
      List.fold_left lib_ctrs ~init:imp_ctr_dict
        ~f:remove_local_id_from_dict
    in
    let contract_dicts =
      {
        ns_dict;
        var_dict = contract_var_dict;
        typ_dict = contract_typ_dict;
        ctr_dict = contract_ctr_dict;
      }
    in
