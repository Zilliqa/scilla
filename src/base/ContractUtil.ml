...
module ContractUtil (Datatypes : Datatype) = struct

  module CUPP = PrettyPrinter (Datatypes)
  module CULiteral = Datatypes.DTLiteral
  open CUPP
  ...
  module ScillaContractUtil (SR : Rep) (ER : Rep) = struct
    ...
    let balance_field =
      (parse_variable_name balance_label ER.uint128_rep, uint128_typ)

    let append_implict_contract_params tparams =
      let creation_block = (parse_variable_name creation_block_label ER.bnum_rep, bnum_typ) in
      let this_address =
        (parse_variable_name this_address_label ER.address_rep, bystrx_typ address_length)
      in
      let scilla_version_init =
        (parse_variable_name scilla_version_label ER.uint32_rep, uint32_typ)
      in
    ...
    let append_implict_comp_params cparams =
      let sender =
        ( parse_variable_name MessagePayload.sender_label ER.address_rep,
          bystrx_typ address_length )
      in
      let amount =
        (parse_variable_name MessagePayload.amount_label ER.uint128_rep, uint128_typ)
      in
      amount :: sender :: cparams

    let msg_mandatory_field_types =
      [
        (MessagePayload.tag_label, string_typ);
        (MessagePayload.amount_label, uint128_typ);
        (MessagePayload.recipient_label, bystrx_typ address_length);
      ]
