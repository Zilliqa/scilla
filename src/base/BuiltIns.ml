  ...
  module BuiltInDictionary = struct
    ...
    *)
    ...
    let built_in_multidict : builtin -> built_in_record list = function
      | Builtin_eq ->
          [
            (String.eq_arity, String.eq_type, elab_id, String.eq);
      ...
      | Builtin_concat ->
          [
            (String.concat_arity, String.concat_type, elab_id, String.concat);
            ( Crypto.concat_arity,
              Crypto.concat_type,
              Crypto.concat_elab,
              Crypto.concat );
      ...
      | Builtin_to_uint256 ->
          [
            ( Crypto.to_uint256_arity,
              Crypto.to_uint256_type,
              Crypto.to_uint256_elab,
              Crypto.to_uint256 );
            ( Uint.to_uint_arity,
              Uint.to_uint_type,
              Uint.to_uint_elab Bits256,
              Uint.to_uint256 );
          ]
      (* Strings *)
      | Builtin_substr ->
          [ (String.substr_arity, String.substr_type, elab_id, String.substr) ]
      | Builtin_strlen ->
          [ (String.strlen_arity, String.strlen_type, elab_id, String.strlen) ]
      | Builtin_to_string ->
          [
            ( String.to_string_arity,
              String.to_string_type,
              String.to_string_elab,
              String.to_string );
          ]
      (* Block numbers *)
      | Builtin_blt -> [ (BNum.blt_arity, BNum.blt_type, elab_id, BNum.blt) ]
      | Builtin_badd ->
          [ (BNum.badd_arity, BNum.badd_type, BNum.badd_elab, BNum.badd) ]
      | Builtin_bsub ->
          [ (BNum.bsub_arity, BNum.bsub_type, BNum.bsub_elab, BNum.bsub) ]
      (* Crypto *)
      | Builtin_sha256hash ->
          [
            ( Crypto.hash_arity,
      ...
      | Builtin_ripemd160hash ->
          [
            ( Crypto.hash_arity,
              Crypto.ripemd160hash_type,
              Crypto.hash_elab,
              Crypto.ripemd160hash );
          ]
      | Builtin_to_bystr ->
          [
            ( Crypto.to_bystr_arity,
              Crypto.to_bystr_type,
              Crypto.to_bystr_elab,
              Crypto.to_bystr );
          ]
      | Builtin_bech32_to_bystr20 ->
          [
            ( Crypto.bech32_to_bystr20_arity,
              Crypto.bech32_to_bystr20_type,
              elab_id,
              Crypto.bech32_to_bystr20 );
          ]
      | Builtin_bystr20_to_bech32 ->
          [
            ( Crypto.bystr20_to_bech32_arity,
              Crypto.bystr20_to_bech32_type,
              elab_id,
              Crypto.bystr20_to_bech32 );
          ]
      | Builtin_schnorr_verify ->
          [
            ( Crypto.schnorr_verify_arity,
              Crypto.schnorr_verify_type,
              elab_id,
              Crypto.schnorr_verify );
          ]
      | Builtin_ecdsa_verify ->
          [
            ( Crypto.ecdsa_verify_arity,
              Crypto.ecdsa_verify_type,
              elab_id,
              Crypto.ecdsa_verify );
          ]
      | Builtin_schnorr_get_address ->
          [
            ( Crypto.schnorr_get_address_arity,
              Crypto.schnorr_get_address_type,
              elab_id,
              Crypto.schnorr_get_address );
          ]
      | Builtin_alt_bn128_G1_add ->
          [
            ( Crypto.alt_bn128_G1_add_arity,
              Crypto.alt_bn128_G1_add_type,
              elab_id,
              Crypto.alt_bn128_G1_add );
          ]
      | Builtin_alt_bn128_G1_mul ->
          [
            ( Crypto.alt_bn128_G1_mul_arity,
              Crypto.alt_bn128_G1_mul_type,
              elab_id,
              Crypto.alt_bn128_G1_mul );
          ]
      | Builtin_alt_bn128_pairing_product ->
          [
            ( Crypto.alt_bn128_pairing_product_arity,
              Crypto.alt_bn128_pairing_product_type,
              elab_id,
              Crypto.alt_bn128_pairing_product );
          ]
      (* Maps *)
      | Builtin_contains ->
          [
      ...
      | Builtin_put ->
          [ (Maps.put_arity, Maps.put_type, Maps.put_elab, Maps.put) ]
      | Builtin_get ->
          [ (Maps.get_arity, Maps.get_type, Maps.get_elab, Maps.get) ]
      | Builtin_remove ->
          [
            (Maps.remove_arity, Maps.remove_type, Maps.remove_elab, Maps.remove);
          ]
      | Builtin_to_list ->
          [
            ( Maps.to_list_arity,
      ...
      | Builtin_size ->
          [ (Maps.size_arity, Maps.size_type, Maps.size_elab, Maps.size) ]
      (* Integers *)
      | Builtin_lt ->
          [
            (Int.eq_arity, Int.eq_type, Int.eq_elab, Int.lt);
            (Uint.eq_arity, Uint.eq_type, Uint.eq_elab, Uint.lt);
          ]
      | Builtin_add ->
          [
            (Int.binop_arity, Int.binop_type, Int.binop_elab, Int.add);
            (Uint.binop_arity, Uint.binop_type, Uint.binop_elab, Uint.add);
          ]
      | Builtin_sub ->
          [
            (Int.binop_arity, Int.binop_type, Int.binop_elab, Int.sub);
            (Uint.binop_arity, Uint.binop_type, Uint.binop_elab, Uint.sub);
          ]
      | Builtin_mul ->
          [
            (Int.binop_arity, Int.binop_type, Int.binop_elab, Int.mul);
            (Uint.binop_arity, Uint.binop_type, Uint.binop_elab, Uint.mul);
          ]
      | Builtin_div ->
          [
            (Int.binop_arity, Int.binop_type, Int.binop_elab, Int.div);
            (Uint.binop_arity, Uint.binop_type, Uint.binop_elab, Uint.div);
          ]
      | Builtin_rem ->
          [
            (Int.binop_arity, Int.binop_type, Int.binop_elab, Int.rem);
            (Uint.binop_arity, Uint.binop_type, Uint.binop_elab, Uint.rem);
          ]
      | Builtin_pow ->
          [
            (Int.pow_arity, Int.pow_type, Int.pow_elab, Int.pow);
      ...
      | Builtin_to_uint128 ->
          [
            ( Uint.to_uint_arity,
              Uint.to_uint_type,
              Uint.to_uint_elab Bits128,
              Uint.to_uint128 );
          ]
      | Builtin_to_nat ->
          [
            (Uint.to_nat_arity, Uint.to_nat_type, Uint.to_nat_elab, Uint.to_nat);
          ]
