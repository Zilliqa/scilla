...
module DataTypeDictionary = struct
  ...
  let add_adt (new_adt : adt) error_loc =
    ...
    | Some _ ->
        fail1
          ~kind:("Multiple declarations of type")
    ...
    | None ->
        ...
        foldM new_adt.tconstr ~init:() ~f:(fun () ctr ->
            ...
            | Some _ ->
                fail1
                  ~kind:"Multiple declarations of type constructor"
  ...
  let lookup_name ?(sloc = ErrorUtils.dummy_loc) name =
    ...
    | None ->
        fail1 ~kind:"ADT not found"
              ~inst:(DTName.as_error_string name)
              sloc
    | Some (_, a) -> pure a
  ...
  let lookup_constructor ?(sloc = ErrorUtils.dummy_loc) cn =
    ...
    | None ->
        fail1
          ~kind:"No data type with this constructor found"
...
module SnarkTypes = struct
  ...
  let scilla_scalar_to_ocaml s =
    ...
    | _ -> fail0 ~kind:"Cannot convert scilla G1 point to ocaml G1 point." ?inst:None
  ...
  let scilla_g2point_to_ocaml g2p =
    ...
    | _ -> fail0 ~kind:"Cannot convert scilla G2 point to ocaml G2 point." ?inst:None
    ...
    | Some x, Some y ->
        ...
        @@ ADTValue
             ( DataTypeDictionary.c_pair.cname,
               ...
               [ ByStrX x; ByStrX y ] )
    | _ -> fail0 ~kind:"Cannot convert OCaml G1 point to Scilla literal." ?inst:None
      ...
      mapM g1g2ol ~f:(fun g1g2p_lit ->
          ...
          | ADTValue (c, [ g1pt; g2pt ], [ g1p; g2p ])
            when is_pair_ctr_name c
              ...
              pure (g1p', g2p')
          | _ ->
              fail0 ~kind:"Cannot convert scilla G1-G2 pair list to ocaml." ?inst:None)
    in
    pure g1g2ol'
end
