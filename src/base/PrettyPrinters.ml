...
module PrettyPrinter (Datatype : Datatype) = struct

  open Datatype
  open Datatype.DTLiteral
  open Datatype.DTLiteral.LType
  ...
  let lookup_constructor_exn cn =
    let t = Datatype.DataTypeDictionary.lookup_constructor cn in
    match t with Error emsg -> raise (InternalError emsg) | Ok s -> s
  ...
  and literal_to_json lit =
    ...
    | Map ((_, _), kvs) -> `List (mapvalues_to_json kvs)
    | ADTValue (n, t, v) as ls ->
        ...
        if String.(a.tname = "List") then
          ...
          match scilla_list_to_ocaml_rev ls with
          | Ok ls' ->
  ...
  let scilla_error_to_jstring ?(pp = true) elist =
    ...
    let j = `Assoc [ ("errors", j'); ("warnings", k') ] in
    if pp then Basic.pretty_to_string j else Basic.to_string j
