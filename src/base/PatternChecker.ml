...
struct
  ...
  let rec pm_check_gas_charge gc =
    ...
    | LogOf g -> CheckedPatternSyntax.SGasCharge.LogOf (pm_check_gas_charge g)
    [@@deriving sexp]
  ...
  let rec pm_check_stmts stmts =
    ...
    | ((s, rep) as srep) :: sts ->
        let%bind checked_s =
          ...
          | RemoteLoad (i, adr, x, is_mutable) ->
              pure @@ (CheckedPatternSyntax.RemoteLoad (i, adr, x, is_mutable), rep)
