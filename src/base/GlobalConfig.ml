...
let rec get_highest_numbered_log files =
  ...
  | file :: files' ->
      let found =
        Str.string_match (Str.regexp "scilla-runner-\\([0-9]+\\)\\.log") file 0
      in
...
module StdlibTracker = struct
  ...
  let get_stdlib_dirs () =
    let env_dirs =
      Option.value_map
        ...
        ~f:FilePath.path_of_string ~default:[]
    in
    List.append env_dirs !stdlib_dirs

  (* Update stdlib dirs with more locations *)
  let add_stdlib_dirs dirs = stdlib_dirs := List.append !stdlib_dirs dirs
  ...
  let find_lib_dir name =
    ...
    BatList.find_opt (fun d -> Caml.Sys.file_exists (d ^/ name ^. file_extn_library)) dirs
end
