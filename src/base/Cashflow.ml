...
module CFName = CFIdentifier.Name
...
module CashflowRep (R : Rep) = struct
  ...
  let rec money_tag_to_string tag =
    match tag with
    | Adt (n, ts) ->
        "(" ^ CFName.as_error_string n ^ " "
        ^ String.concat ~sep:" " (List.map ~f:money_tag_to_string ts)
        ^ ")"
...
struct
  ...
  let add_money_or_mapmoney_to_ident i typ =
    ...
    let rec create_money_tag typ =
      ...
      | _ -> ECFR.Money
    in
    add_tag_to_id i (create_money_tag typ)
  ...
  let rec cf_init_tag_pattern p =
    ...
    | Constructor (cn, ps) ->
        CFSyntax.Constructor (cn, List.map ~f:cf_init_tag_pattern ps)
  ...
  let rec cf_init_tag_expr erep =
    ...
    let res_e =
      ...
      | Message bs ->
          CFSyntax.Message
            (List.map ~f:(fun (s, p) -> (s, cf_init_tag_payload p)) bs)
    in
      ...
      | MapUpdate (m, ks, v) ->
          CFSyntax.MapUpdate
            ( add_noinfo_to_ident m,
              ...
              match v with
              | None -> None
              | Some v' -> Some (add_noinfo_to_ident v') )
  ...
  let cf_init_tag_library lib =
    ...
    let init_tag_entry entry =
      ...
      | LibVar (lname, ltype, lexp) ->
          CFSyntax.LibVar
            (add_noinfo_to_ident lname, ltype, cf_init_tag_expr lexp)
      | LibTyp (lname, type_defs) ->
          CFSyntax.LibTyp
            ( add_noinfo_to_ident lname,
              List.map ~f:cf_init_tag_type_def type_defs )
    in
                ...
                let rec match_arg_tag_with_typ arg_typ arg_tag targ_tag_map =
                  ...
                  | ADT (_, adt_params) -> (
                      ...
                      | Adt (_, adt_param_tags) ->
                          let new_map, _ =
                            List.fold_left adt_params
                              ~init:(targ_tag_map, adt_param_tags)
  ...
  let lookup_var_tag i env =
    let open CFIdentifier in
    match AssocDictionary.lookup (as_string i) env with
    | Some t -> t
    | None -> get_id_tag i

  let lookup_var_tag2 i env1 env2 =
    let open CFIdentifier in
    match AssocDictionary.lookup (as_string i) env1 with
    | Some t -> t
    | None -> lookup_var_tag i env2

  let update_var_tag i t env =
    AssocDictionary.update (CFIdentifier.as_string i) t env

  let update_var_tag2 i t env1 env2 =
    match AssocDictionary.lookup (CFIdentifier.as_string i) env1 with
    ...
    let (c_r, c_as) =
      ...
      | Builtin_get ->
          let c_r_sigs =
            match res_tag with
            | Adt (adt_name, [t])
              when is_option_adt_name adt_name ->
                                     [ (option_adt_tag t     , [ Map t      ; NotMoney ] ) ]
            | NoInfo              -> [ (option_adt_tag NoInfo, [ Map NoInfo ; NotMoney ] ) ]
            | _                   -> [ ( Inconsistent        , [ Map NoInfo ; NotMoney ] ) ] in
          let c_as_sigs =
            match args_tags with
            | [ m ; k ] ->
                let m_sig =
                  match m with
                  | Map t  -> [ ( option_adt_tag t     , [ Map t        ; NotMoney ] ) ] 
                  | NoInfo -> [ ( option_adt_tag NoInfo, [ Map NoInfo   ; NotMoney ] ) ] 
                  | _      -> [ ( option_adt_tag NoInfo, [ Inconsistent ; NotMoney ] ) ] in
                let k_sig =
                  match k with
                  | NotMoney
                  | NoInfo   -> [ ( option_adt_tag NoInfo, [ Map NoInfo ; NotMoney     ] ) ]
                  | _        -> [ ( option_adt_tag NoInfo, [ Map NoInfo ; Inconsistent ] ) ] in
                [ m_sig ; k_sig ]
            | _             ->
                (* Error *)
                [[ ( Inconsistent , List.map ~f:(fun _ -> Inconsistent) args_tags ) ]] in
  ...
  let update_pattern_vars_tags_from_usage_and_remove_from_env p local_env
      ctr_tag_map =
    let rec walk p local_env ctr_tag_map =
      ...
      | Constructor (s, ps) ->
          ...
          let new_ctr_tag_map, ctr_tag_map_changes =
            update_ctr_tag_map ps_ctr_tag_map (CFIdentifier.as_string s)
              new_ps_tags
            |> Option.value_map ~default:(ps_ctr_tag_map, false) ~f:(fun map ->
                   (map, true))
          in
          let ctr_tag = ctr_to_adt_tag (CFIdentifier.get_id s) new_ps_tags in
  ...
  let insert_pattern_vars_into_env p local_env =
    ...
    List.fold_left pattern_vars ~init:local_env ~f:(fun l_env x ->
        AssocDictionary.insert (CFIdentifier.as_string x) (get_id_tag x) l_env)
  ...
  let lub_pattern_tags ps =
    let rec walk acc_tag p =
      ...
      | Constructor (s, ps) ->
          let expected_subtags =
            match ctr_pattern_to_subtags (CFIdentifier.get_id s) acc_tag with
            | Some ts -> ts
            | None -> List.map ps ~f:(fun _ -> NoInfo)
          in
          let subpattern_tags =
  ...
  let update_var_tag_payload p local_env =
    ...
    | MVar v -> (
        let tag =
          match AssocDictionary.lookup (CFIdentifier.as_string v) local_env with
    ...
    let ( new_e,
      ...
      | Fun (arg, t, body) ->
          ...
          let ( ((_, (new_body_tag, _)) as new_body),
                ...
                res_ctr_tag_map,
                body_changes ) =
            cf_tag_expr body body_expected_tag param_env body_local_env
              ctr_tag_map
          in
      ...
      | App (f, args) ->
          ...
          in
          let f_tag =
            lub_tags (lookup_var_tag f local_env) (Map expected_tag)
          in
      ...
      | Builtin (f, args) ->
          let args_tags =
            List.map
              ...
              args
          in
          let res_tag, args_tags_usage =
            builtin_signature f expected_tag args_tags
          in
      ...
      | Let (i, topt, lhs, rhs) ->
          ...
          let updated_lhs_local_env =
            AssocDictionary.insert (CFIdentifier.as_string i) new_lhs_tag
              lhs_local_env
          in
          ...
          let new_i = update_id_tag i new_i_tag in
          let res_local_env =
            AssocDictionary.remove (CFIdentifier.as_string i) rhs_local_env
          in
            ...
            List.fold_right clauses
              ...
              ~f:(fun (p, ep)
                ...
                let ( ((_, (new_e_tag, _)) as new_e),
                      ...
                      e_ctr_tag_map,
                      new_changes ) =
                  cf_tag_expr ep expected_tag acc_param_env sub_local_env
                    acc_ctr_tag_map
                in
                let new_p, new_local_env, new_ctr_tag_map, p_changes =
                  update_pattern_vars_tags_from_usage_and_remove_from_env p
                    new_local_env e_ctr_tag_map
                in
            ...
            List.fold_right bs ~init:([], param_env, local_env, false)
              ~f:(fun (s, p)
                ...
                | MVar x ->
                    ...
                    in
                    let old_env_tag =
                      lookup_var_tag2 x acc_local_env acc_param_env
                    in
  ...
  let cf_update_tag_for_field_assignment f x param_env field_env local_env =
    ...
    let new_local_env, new_param_env =
      update_var_tag2 x new_tag local_env param_env
    in
  ...
  let rec cf_tag_stmt (srep : CFSyntax.stmt_annot) param_env field_env local_env
    ...
    let ( new_s,
      ...
      | Load (x, f) ->
          ...
          let new_local_env =
            AssocDictionary.remove (CFIdentifier.as_string x) tmp_local_env
          in
          ( Load (new_x, new_f),
            new_param_env,
            new_field_env,
      ...
      | MapUpdate (m, ks, v_opt) ->
          ...
          in
          let m_tag_usage =
            List.fold_left ks ~init:v_tag ~f:(fun acc _ -> Map acc)
          in
          ...
          let ks_local_env, ks_param_env =
            update_vars_tags2 new_ks local_env param_env
          in
      ...
      | MapGet (x, m, ks, fetch) ->
          ...
          in
          let m_tag_usage =
            List.fold_left ks ~init:val_tag ~f:(fun acc _ -> Map acc)
          in
          let m_tag = lub_tags m_tag_usage (lookup_var_tag m field_env) in
          let new_m = update_id_tag m m_tag in
          let new_field_env = update_var_tag m m_tag field_env in
          ...
          let new_x_tag =
            if fetch then lub_tags x_tag (option_adt_tag new_val_tag)
            else NotMoney
            (* Bool *)
          in
          let new_x = update_id_tag x new_x_tag in
    ...
    let init_local_env =
      List.fold_left ss ~init:local_env ~f:(fun acc_env srep ->
          ...
          | Load (x, _) | Bind (x, _) | MapGet (x, _, _, _) | ReadFromBC (x, _)
            ->
              AssocDictionary.insert (CFIdentifier.as_string x) (get_id_tag x)
                acc_env
          | _ -> acc_env)
    in
    List.fold_right ss
      ...
      ~f:(fun s
        ...
        let ( new_s,
              ...
              new_ctr_tag_map,
              new_changes ) =
          cf_tag_stmt s acc_param_env acc_field_env acc_local_env
            acc_ctr_tag_map
        in
        ( new_s :: acc_ss,
          new_param_env,
  ...
  let cf_tag_contract c =
        ...
        ~f:(fun acc_env (f, _, e) ->
          let (_, (e_tag, _)), _, _, _, _ =
            cf_tag_expr e
              ...
              ctr_tag_map
          in
          AssocDictionary.insert (CFIdentifier.as_string f) e_tag acc_env)
    in
    let rec tagger cconstraint components param_env field_env ctr_tag_map =
      ...
      if changes then
        ...
        (new_constraint, new_ts, new_param_env, new_field_env, tmp_ctr_tag_map)
    in
    let ( new_constraint,
          new_ccomps,
          new_param_env,
  ...
  let main cmod token_fields =
    ...
    let param_field_tags =
      List.map
        ~f:(fun (p, _) -> (CFIdentifier.as_string p, get_id_tag p))
        new_mod.contr.cparams
      @ List.map
          ~f:(fun (f, _, _) -> (CFIdentifier.as_string f, get_id_tag f))
