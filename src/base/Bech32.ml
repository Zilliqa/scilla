...
let bech32_polymod_step pre =
  let b = pre lsr 25 in
  ((pre land 0x1FFFFFF) lsl 5)
  lxor (-((b lsr 0) land 1) land 0x3b6a57b2)
  lxor (-((b lsr 1) land 1) land 0x26508e6d)
  lxor (-((b lsr 2) land 1) land 0x1ea119fa)
  lxor (-((b lsr 3) land 1) land 0x3d4233dd)
...
let decode_bech32_addr ~prefix ~addr:str =
  ...
  then None
      ...
      List.fold (String.to_list prefix)
        ...
        ~f:(fun (chk, err, (have_lower, have_upper)) ch ->
          if err || ascii_of_char ch < 33 || ascii_of_char ch > 126 then
            (chk, true, (have_lower, have_upper))
          else
            let ch', have_lower', have_upper' =
              if
                ascii_of_char ch >= ascii_of_char 'a'
                && ascii_of_char ch <= ascii_of_char 'z'
              then (ascii_of_char ch, true, have_upper)
              else if
                ascii_of_char ch >= ascii_of_char 'A'
                && ascii_of_char ch <= ascii_of_char 'Z'
              then
                ( ascii_of_char ch - ascii_of_char 'A' + ascii_of_char 'a',
                  have_lower,
                  true )
              else (ascii_of_char ch, have_lower, have_upper)
            in
    ...
    if err || (have_lower && have_upper) then None
      ...
      let chk, err, (have_lower, have_upper), _ =
        List.fold
          ...
          ~f:(fun (chk, err, (have_lower, have_upper), index) c ->
            ...
            if err then (chk, err, (have_lower, have_upper), index + 1)
            else
              let c' =
                (* it's an error if any bit other than the least 7 bits are 1. *)
                if ascii_of_char c land (Int.minus_one lsl 7) <> 0 then -1
                else
                  (* charset_rev contains -1 for invalid bech32 characters. *)
                  charset_rev.(ascii_of_char c)
              in
              (* Check for errors so far. *)
              if c' = -1 then (chk, true, (have_lower, have_upper), index + 1)
              else
                (* Do checksum on c' *)
      ...
      in
      if err || (have_lower && have_upper) then None
      else if chk = 1 then Some (string_of_bitstring (Buffer.contents bitacc))
      else None
  ...
  if String.length bys <> 20 then None
    ...
    if err then None
      ...
      let chk, err, bech32_str =
        List.fold (String.to_list input_padded) ~init:(chk, false, "")
          ~f:(fun (chk, err, bech32_acc) ch ->
            (* We should able to use "ch" to index into charset. *)
            if err || ascii_of_char ch lsr 5 <> 0 then (chk, true, bech32_acc)
            else
              (* Checksum *)
