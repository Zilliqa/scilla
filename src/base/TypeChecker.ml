...
module ScillaTypechecker (SR : Rep) (ER : Rep) = struct
  ...
  let wrap_type_error_with_info (msg, sloc) res =
    ...
    | Error (TypeError, e, g) ->
        Error
          (TypeError, { emsg = msg; startl = sloc; endl = dummy_loc } :: e, g)
    | Error (GasError, e, g) ->
  ...
  let assign_types_for_pattern sctyp pattern =
    let rec go atyp tlist p =
      ...
      | Constructor (cn, ps) ->
          ...
          let%bind () = validate_param_length (get_id cn) plen alen in
  ...
  let rec type_expr tenv (erep : UntypedSyntax.expr_annot) remaining_gas =
    ...
    | App (f, actuals) ->
        ...
        @@ let%bind fres =
             mark_error_as_type_error remaining_gas
             @@ TEnv.resolveT tenv (as_string f) ~lopt:(Some (get_rep f))
           in
    ...
    | MatchExpr (x, clauses) ->
        if List.is_empty clauses then
          ...
          let%bind sctyp =
            mark_error_as_type_error remaining_gas
            @@ TEnv.resolveT tenv (as_string x) ~lopt:(Some (get_rep x))
          in
    ...
    | TApp (tf, arg_types) ->
        ...
        let%bind tfres =
          mark_error_as_type_error remaining_gas
          @@ TEnv.resolveT tenv (as_string tf) ~lopt:(Some (get_rep tf))
        in
  ...
  and type_actuals tenv actuals remaining_gas =
    let%bind tresults =
      mapM actuals ~f:(fun arg ->
          mark_error_as_type_error remaining_gas
          @@ TEnv.resolveT tenv (as_string arg) ~lopt:(Some (get_rep arg)))
    in
  ...
  let type_map_access env m' keys' remaining_gas =
    let%bind t' =
      mark_error_as_type_error remaining_gas
      @@ TEnv.resolveT env.fields (as_string m') ~lopt:(Some (get_rep m'))
    in
    let rec helper t keys =
      ...
      | MapType (kt, vt), k :: rest ->
          ...
          in
          let%bind () = assert_type_equiv kt (rr_typ k_t).tp in
    ...
    | ((s, rep) as stmt) :: sts -> (
              ...
              let%bind checked_stmts, f_type, r_type, remaining_gas =
                wrap_type_serr stmt
                  (let%bind fr =
                   ...
                   let%bind r =
                     mark_error_as_type_error remaining_gas
                     @@ TEnv.resolveT env.pure (as_string r)
                          ~lopt:(Some (get_rep r))
                   in
                   let%bind () =
                     mark_error_as_type_error remaining_gas
        ...
        | MatchStmt (x, clauses) ->
            if List.is_empty clauses then
              ...
              let msg =
                sprintf "Error in pattern matching \"%s\" of type %s" (as_error_string x)
                  (pp_typ sct)
              in
              ...
              let%bind checked_clauses_rev, remaining_gas =
                ...
                @@ foldM clauses ~init:([], remaining_gas)
                     ~f:(fun (checked_clauses_acc, remaining_gas) (ptrn, ex) ->
                       ...
                       in
                       pure
                       @@ (typed_clause :: checked_clauses_acc, remaining_gas'))
              in
        ...
        | CallProc (p, args) ->
            let msg =
              sprintf "Type error(s) in procedure call %s:\n" (as_error_string p)
            in
  ...
  let type_component env0 tr remaining_gas :
      ( (TypedSyntax.component * (TCName.t * TCType.t list) list) * Stdint.uint64,
                  ...
                  let env' =
                    TEnv.filterTs (TEnv.copy t_env) ~f:(fun name _ ->
                        List.exists t_lib.lentries ~f:(function
                          | LibTyp _ -> false
                          | LibVar (i, _, _) -> String.(as_string i = name))
         ...
         foldM
           ...
           ~f:(fun ((comp_acc, proc_acc, remaining_gas'), emsgs) tr ->
             ...
             | Error (TypeError, el, g) ->
                 Ok ((comp_acc, proc_acc, g), emsgs @ el)
             | Error (GasError, el, g) -> Error (GasError, el, g))
       in
