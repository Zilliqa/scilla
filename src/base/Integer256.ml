...
module Uint256 = struct
  ...
  let shift_left a shift =
    if shift >= 256 then zero
    else if shift = 128 then { high = a.low; low = Uint128.zero }
    else if shift = 0 then a
    else if shift < 128 then
      let high =
        Uint128.add
          (Uint128.shift_left a.high shift)
          (Uint128.shift_right a.low (128 - shift))
      in
  ...
  let shift_right a shift =
    if shift >= 256 then zero
    else if shift = 128 then { high = Uint128.zero; low = a.high }
    else if shift = 0 then a
    else if shift < 128 then
      ...
      let low =
        Uint128.add
          ...
          (Uint128.shift_right a.low shift)
      in
      { high; low }
    else
      (* shift is greater than 128 *)
  ...
  let lognot a = { high = Uint128.lognot a.high; low = Uint128.lognot a.low }
   ...
   *)
  let mul a b =
    let a0 = getlow a.low in
    let a1 = gethigh a.low in
    let a2 = getlow a.high in
    let a3 = gethigh a.high in
    let b0 = getlow b.low in
    let b1 = gethigh b.low in
    let b2 = getlow b.high in
    let b3 = gethigh b.high in
    (* first row *)
    ...
    then
      ...
      let rec divloop q r i =
        ...
        if i < 0 then (q, r)
          ...
          let q', r''' =
            (* if R ≥ D then *)
            if compare r'' b >= 0 (* R := R − D; Q(i) := 1 *) then
              (setbit q i, sub r'' b)
            else (q, r'')
          in
          (* next iteration *)
  ...
  let [@warning "-32"] of_uint32 a = { low = Uint128.of_uint32 a; high = Uint128.zero }

  let [@warning "-32"] of_uint64 a = { low = Uint128.of_uint64 a; high = Uint128.zero }

  let [@warning "-32"] of_uint128 a = { low = Uint128.of_uint128 a; high = Uint128.zero }
end
...
module Int256 = struct
  ...
  let shift_left a shift = Uint256.shift_left a shift
  ...
  (* Set bit i of a. i=0 will set the least significant bit. *)
  let setbit a i = Uint256.setbit a i

  (* Clear bit i of a. i=0 will clear the least significant bit. *)
  let clearbit a i = Uint256.clearbit a i
  ...
  let abs a = if isneg a then neg a else a
  ...
  let of_string s =
    ...
    else if n then neg i
    else i

  let to_string i =
    if isneg i then "-" ^ Uint256.to_string (neg i) else Uint256.to_string i
