...
module ScillaSyntax (SR : Rep) (ER : Rep) (Name : QualifiedName) = struct
  ...
  let rec subst_type_in_expr tvar tp erep =
    let e, rep = erep in
    match e with
    | Literal l -> (Literal (SLiteral.subst_type_in_literal tvar tp l), rep)
    | Var _ as v -> (v, rep)
    | Fun (f, t, body) ->
        let t_subst = SType.subst_type_in_type' tvar tp t in
    ...
    | Constr (n, ts, es) ->
        let ts' = List.map ts ~f:(fun t -> SType.subst_type_in_type' tvar tp t) in
        (Constr (n, ts', es), rep)
    | App _ as app -> (app, rep)
    | Builtin _ as bi -> (bi, rep)
    | Let (i, tann, lhs, rhs) ->
        let tann' =
          Option.map tann ~f:(fun t -> SType.subst_type_in_type' tvar tp t)
        in
    ...
    | MatchExpr (e, cs) ->
        ...
        (MatchExpr (e, cs'), rep)
    | TApp (tf, tl) ->
        let tl' = List.map tl ~f:(fun t -> SType.subst_type_in_type' tvar tp t) in
        (TApp (tf, tl'), rep)
    | Fixpoint (f, t, body) ->
        let t' = SType.subst_type_in_type' tvar tp t in
