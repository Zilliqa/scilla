...
module ScillaCallgraph (SR : Rep) (ER : Rep) = struct
  ...
  module Edge = struct
    type 'node t = { src_node : 'node; dst_node : 'node }

    let mk src_node dst_node = { src_node; dst_node }
    let src edge = edge.src_node
    let dst edge = edge.dst_node
  end

  module Node = struct
    type node_ty = Transition | Procedure | Function
  ...
  let collect_nodes (cmod : cmodule) =
    let collect_lib_functions (lib : library) =
      (* TODO: Should we handle tapps here? *)
      let rec aux init lib =
        List.fold_left lib.lentries ~init ~f:(fun acc lentry ->
            ...
            in
            match lentry with
            (* NOTE: LibVar's type will be set only there is a user-defined type
                     annotation. Type inferrence doesn't set the type here. *)
    ...
    let collect_comps cmod =
      List.fold_left ~init:[] cmod.contr.ccomps ~f:(fun acc comp ->
          let ty =
            ...
            | CompProc -> Node.Procedure
            | CompTrans -> Node.Transition
          in
  ...
  let rec collect_funcalls (e, _annot) collected_nodes =
    ...
    | Literal _ | Var _ | Message _ | Constr _ | Builtin _ | TApp _ -> []
  ...
  module GDot = Graph.Graphviz.Dot (struct
    ...
    let vertex_attributes (v : V.t) =
      match v.ty with
      | Node.Transition -> [ `Shape `Box; `Style `Filled; `Fillcolor 0xd3869b ]
      | Node.Procedure -> [ `Shape `Box; `Style `Filled; `Fillcolor 0x83a598 ]
      | Node.Function -> [ `Shape `Box; `Style `Filled; `Fillcolor 0x8ec07c ]
