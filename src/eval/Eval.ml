...
(* Replace address types with ByStr20 in a type. *)
let rec erase_address_types_in_type t =
  match t with
  | PrimType _ | TypeVar _ | PolyFun _ | Unit -> t
  | Address _ -> bystrx_typ Type.address_length
  | MapType (kt, vt) ->
      MapType (erase_address_types_in_type kt, erase_address_types_in_type vt)
  | FunType (t1, t2) ->
      FunType (erase_address_types_in_type t1, erase_address_types_in_type t2)
  | ADT (tname, targs) ->
      ADT (tname, List.map targs ~f:erase_address_types_in_type)
...
let erase_address_types_in_literal l =
  match l with
  ...
  | Map ((kt, vt), tbl) ->
      ...
      Map ((erase_address_types_in_type kt, erase_address_types_in_type vt), tbl)
  | ADTValue (cname, targs, vargs) ->
      (* vargs sanitised before ADTValue is constructed. Only need to handle type arguments *)
      ADTValue (cname, List.map targs ~f:erase_address_types_in_type, vargs)
...
let rec exp_eval erep env =
  let e, loc = erep in
  match e with
  | Literal l -> pure (erase_address_types_in_literal l, env)
  | Var i ->
  ...
  | Constr (cname, ts, actuals) ->
      ...
      if constr.arity <> alen then
        ...
        pure (erase_address_types_in_literal lit, env)
  | MatchExpr (x, clauses) ->
