...
let default_proto_scilla_val_map_mutable () : proto_scilla_val_map_mutable = { m = [] }
...
and decode_proto_scilla_val d =
  let rec loop () =
    let ret : Ipcmessage_types.proto_scilla_val =
      ...
      | Some (2, _) ->
          Ipcmessage_types.Mval (decode_proto_scilla_val_map (Pbrt.Decoder.nested d))
      | Some (n, payload_kind) ->
...
let rec encode_proto_scilla_val_map (v : Ipcmessage_types.proto_scilla_val_map) encoder =
  ...
  let encode_value x encoder = Pbrt.Encoder.nested (encode_proto_scilla_val x) encoder in
  List.iter
    (fun (k, v) ->
