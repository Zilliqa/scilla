...
type g2point = { g2x : string; g2y : string } (* TODO: Replace with (ByStr64,ByStr64) *)

let scalar_len = 32
let g1point_len = scalar_len + scalar_len
let g2comp_len = 64
let g2point_len = g2comp_len + g2comp_len
let g1g2pair_len = g1point_len + g2point_len (* each pair in alt_bn128_pairing_product *)

let encode_g1point_bytes g1 = (g1.g1x ^ g1.g1y)
let decode_g1point_bytes g1b =
  if String.length g1b <> g1point_len then None else
...
let alt_bn128_G1_add p1 p2 =
  ...
  if String.length p1.g1x <> scalar_len || String.length p1.g1y <> scalar_len ||
     String.length p2.g1x <> scalar_len || String.length p2.g1y <> scalar_len
  then None else
  ...
  let pres =
    if succ
    then decode_g1point_bytes (copy_from_tstring (string_from_ptr presD ~length:g1point_len))
    else None
  in
...
let alt_bn128_G1_mul p s =
  ...
  (* This check can be removed once we have a strong type for scalar. *)
  if String.length p.g1x <> scalar_len || String.length p.g1y <> scalar_len
...
let alt_bn128_pairing_product pairs =
  ...
  let alt_bn128_pairing_product_Z =
    foreign "alt_bn128_pairing_product_Z"
      (ptr rawBytes_Z @-> ptr rawBytes_Z @-> returning bool) in
  ...
  (* Copy input data to input structs. *)
  let pairs' = String.concat "" (List.map (fun (g1p, g2p) -> encode_g1g2pair_bytes g1p g2p) pairs) in
...
let eq_scalar = (=) (* Replace with proper operator once we have a string type. *)
