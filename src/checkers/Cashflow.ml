...
module CashflowRep (R : Rep) = struct
  ...
  let rec money_tag_to_string tag =
    match tag with
    | Adt (n, ts) ->
        "(" ^ n ^ " " ^ String.concat ~sep:" " (List.map ~f:money_tag_to_string ts) ^ ")"
  ...
  let add_tag_to_id s tag = match s with Ident (n, r) -> Ident (n, (tag, r))
...
struct
  ...
  let add_money_or_mapmoney_to_ident i typ =
    let rec create_money_tag typ =
      match typ with
      | MapType (_, vtyp) -> ECFR.Map (create_money_tag vtyp)
      | _ -> ECFR.Money
    in
  ...
  let rec cf_init_tag_pattern p =
    ...
    | Constructor (cn, ps) -> CFSyntax.Constructor (cn, List.map ~f:cf_init_tag_pattern ps)
  ...
  let rec cf_init_tag_expr erep =
    ...
    let res_e =
      match e with
      | Literal l -> CFSyntax.Literal l
      | Var i -> CFSyntax.Var (add_noinfo_to_ident i)
      | Fun (arg, t, body) ->
          CFSyntax.Fun (add_noinfo_to_ident arg, t, cf_init_tag_expr body)
      | App (f, actuals) ->
          CFSyntax.App (add_noinfo_to_ident f, List.map ~f:add_noinfo_to_ident actuals)
      | Builtin (op, actuals) ->
          CFSyntax.Builtin
            (add_noinfo_to_builtin op, List.map ~f:add_noinfo_to_ident actuals)
      | Let (i, topt, lhs, rhs) ->
          CFSyntax.Let
            (add_noinfo_to_ident i, topt, cf_init_tag_expr lhs, cf_init_tag_expr rhs)
      | Constr (cname, ts, actuals) ->
          CFSyntax.Constr (cname, ts, List.map ~f:add_noinfo_to_ident actuals)
      | MatchExpr (x, clauses) ->
          CFSyntax.MatchExpr
            ( add_noinfo_to_ident x,
              List.map
                ~f:(fun (p, e) -> (cf_init_tag_pattern p, cf_init_tag_expr e))
                clauses )
      | Fixpoint (f, t, body) ->
          CFSyntax.Fixpoint (add_noinfo_to_ident f, t, cf_init_tag_expr body)
      | TFun (tvar, body) ->
          CFSyntax.TFun (add_noinfo_to_ident tvar, cf_init_tag_expr body)
      | TApp (tf, arg_types) -> CFSyntax.TApp (add_noinfo_to_ident tf, arg_types)
      | Message bs ->
          CFSyntax.Message (List.map ~f:(fun (s, p) -> (s, cf_init_tag_payload p)) bs)
    in
  ...
  let rec cf_init_tag_stmt srep =
    ...
    let res_s =
      match s with
      | Load (x, y) -> CFSyntax.Load (add_noinfo_to_ident x, add_noinfo_to_ident y)
      | Store (x, y) -> CFSyntax.Store (add_noinfo_to_ident x, add_noinfo_to_ident y)
      ...
      | CallProc (p, args) -> CFSyntax.CallProc (p, List.map args ~f:add_noinfo_to_ident)
      | Throw xopt -> (
          match xopt with
        ...
        List.map
          ~f:(fun (x, t, e) ->
            ( ( if token_fields_contains x then add_money_or_mapmoney_to_ident x t
              ...
              cf_init_tag_expr e ))
          cfields;
      CFSyntax.ccomps = List.map ~f:cf_init_tag_component ccomps;
    }

  let cf_init_tag_type_def tdef =
    let { cname; c_arg_types } = tdef in
    { CFSyntax.cname = add_noinfo_to_ident cname; CFSyntax.c_arg_types }
  ...
  let cf_init_tag_library lib =
    ...
    let init_tag_entry entry =
      ...
      | LibTyp (lname, type_defs) ->
          CFSyntax.LibTyp
            (add_noinfo_to_ident lname, List.map ~f:cf_init_tag_type_def type_defs)
    in
        ...
        | _ -> (
            ...
            | Some arg_typs ->
                ...
                let init_targ_to_tag_map =
                  List.map adt.tparams ~f:(fun tparam -> (tparam, NoInfo))
                in
                          ...
                          let new_map, _ =
                            List.fold_left adt_params ~init:(targ_tag_map, adt_param_tags)
                              ~f:(fun (map, adt_param_tags) adt_param ->
                                ...
                                | [] -> (map, [])
                                | t :: ts -> (match_arg_tag_with_typ adt_param t map, ts))
                          in
                          new_map
                      | _ -> targ_tag_map )
                ...
                let tvar_tag_map, _ =
                  List.fold_left arg_typs ~init:(init_targ_to_tag_map, arg_tags)
                    ~f:(fun (map, arg_tags) arg_typ ->
                      match arg_tags with
                      | [] -> (map, [])
                      | t :: ts -> (match_arg_tag_with_typ arg_typ t map, ts))
                in
    ...
    | Ok (adt, _) -> (
        ...
        | Some tmap ->
            let tvar_tag_map =
              let zipped_tvar_tags =
                ...
                | _ ->
                    ...
                    Ok (List.map adt.tparams ~f:(fun tparam -> (tparam, NoInfo)))
              in
            ...
            let rec tag_tmap t =
              ...
              | ADT (adt_name, arg_typs) -> Adt (adt_name, List.map arg_typs ~f:tag_tmap)
              | TypeVar tvar -> (
  ...
  let ctr_arg_filter targ =
    ...
    | TypeVar _
    (* TypeVars tagged at type level *)  | PolyFun _ (* Ignore *) | Unit ->
        (* Ignore *)
        false
  ...
  let init_ctr_tag_map () =
    ...
    List.fold_left all_ctrs ~init:[] ~f:(fun acc (adt, ctr) ->
        ...
        | Some targs ->
            let tag_list =
              List.map targs ~f:(fun t -> if ctr_arg_filter t then Some NoInfo else None)
            in
            if List.exists tag_list ~f:Option.is_some then (ctr.cname, tag_list) :: acc
            else acc)
  ...
  let get_id_tag id = match id with Ident (_, (tag, _)) -> tag

  let update_id_tag id new_tag =
    match id with Ident (v, (_, rep)) -> Ident (v, (new_tag, rep))

  let lookup_var_tag i env =
    match AssocDictionary.lookup (get_id i) env with Some t -> t | None -> get_id_tag i
  ...
  let update_var_tag i t env = AssocDictionary.update (get_id i) t env
  ...
  let update_vars_tags2 ids env1 env2 =
    List.fold_left ids ~init:(env1, env2) ~f:(fun (acc_env1, acc_env2) id ->
  ...
  let rec get_pattern_vars p acc =
    ...
    | Wildcard -> acc
    | Binder x -> x :: acc
    | Constructor (_, ps) -> List.fold_right ps ~init:acc ~f:get_pattern_vars
    ...
    let rec walk p local_env ctr_tag_map =
      ...
      | Constructor (s, ps) ->
          ...
          ( Constructor (s, new_ps),
            ...
            new_local_env,
            new_ctr_tag_map,
            ctr_tag_map_changes || ps_changes )
    in
    let new_p, _, new_local_env, new_ctr_tag_map, changes =
      walk p local_env ctr_tag_map
    in
    ...
    let rec walk p expected_tag =
      ...
      | Constructor (s, ps) ->
          ...
          let new_subpatterns_with_tags, _ =
            List.zip_with_remainder ps expected_subtags
          in
          let new_ps, changes =
            List.fold_right new_subpatterns_with_tags ~init:([], false)
              ~f:(fun (p, exp_tag) (acc_ps, acc_changes) ->
                let new_p, change = walk p exp_tag in
                (new_p :: acc_ps, acc_changes || change))
          in
          (Constructor (s, new_ps), changes)
    in
    walk p scrutinee_tag

  let insert_pattern_vars_into_env p local_env =
    let pattern_vars = get_pattern_vars p [] in
    List.fold_left pattern_vars ~init:local_env ~f:(fun l_env x ->
  ...
  let update_var_tag_payload p local_env =
    ...
    | MVar v -> (
        let tag =
          ...
          | Some t -> t
        in
        match v with Ident (name, (_, rep)) -> CFSyntax.MVar (Ident (name, (tag, rep))) )
    ...
    let new_e, new_e_tag, new_param_env, new_local_env, new_ctr_tag_map, new_changes =
      ...
      | Var i ->
          ...
          let new_local_env, new_param_env =
            update_var_tag2 i new_i_tag local_env param_env
          in
          ( Var new_i,
            new_i_tag,
            new_param_env,
      ...
      | Fun (arg, t, body) ->
          ...
          let body_expected_tag =
            match expected_tag with Map x -> x | NoInfo -> NoInfo | _ -> Inconsistent
          in
          let body_local_env =
            AssocDictionary.insert (get_id arg) (get_id_tag arg) local_env
          in
          let ( ((_, (new_body_tag, _)) as new_body),
                ...
                res_ctr_tag_map,
                body_changes ) =
            cf_tag_expr body body_expected_tag param_env body_local_env ctr_tag_map
          in
      ...
      | App (f, args) ->
          let new_args =
            List.map
              ~f:(fun arg -> update_id_tag arg (lookup_var_tag2 arg local_env param_env))
              args
          in
          let args_changes =
            match
          ...
          let new_e_tag =
            match f_tag with Map t -> t | NoInfo -> NoInfo | _ -> Inconsistent
          in
          ( App (new_f, new_args),
            new_e_tag,
            param_env,
      ...
      | Builtin (f, args) ->
          let args_tags =
            List.map ~f:(fun arg -> lookup_var_tag2 arg local_env param_env) args
          in
      ...
      | Let (i, topt, lhs, rhs) ->
          ...
          let updated_lhs_local_env =
            AssocDictionary.insert (get_id i) new_lhs_tag lhs_local_env
          in
          let ( ((_, (new_rhs_tag, _)) as new_rhs),
                ...
                rhs_ctr_tag_map,
                rhs_changes ) =
            cf_tag_expr rhs expected_tag lhs_param_env updated_lhs_local_env
              lhs_ctr_tag_map
          in
      ...
      | Constr (cname, ts, args) ->
          ...
          let new_ctr_tag_map, ctr_tag_map_changes =
            ...
            |> Option.value_map ~default:(ctr_tag_map, false) ~f:(fun map -> (map, true))
          in
      ...
      | MatchExpr (x, clauses) ->
            ...
            List.fold_right clauses
              ...
              ~f:(fun (p, ep)
                      ( acc_clauses,
                        ...
                        acc_changes )
                      ->
                let sub_local_env = insert_pattern_vars_into_env p acc_local_env in
                let ( ((_, (new_e_tag, _)) as new_e),
                      new_param_env,
                      new_local_env,
                      e_ctr_tag_map,
                      new_changes ) =
                  cf_tag_expr ep expected_tag acc_param_env sub_local_env acc_ctr_tag_map
                in
                let new_p, new_local_env, new_ctr_tag_map, p_changes =
                  update_pattern_vars_tags_from_usage_and_remove_from_env p new_local_env
                    e_ctr_tag_map
                in
          ...
          let x_res_clauses, clause_changes =
            List.fold_right res_clauses ~init:([], res_clause_changes)
              ~f:(fun (p, e) (acc_ps, acc_changes) ->
                let new_p, p_change =
                  update_pattern_vars_tags_from_scrutinee p (get_id_tag x)
                in
                ((new_p, e) :: acc_ps, p_change || acc_changes))
          in
          let x_tag_usage =
            lub_pattern_tags (List.map ~f:(fun (p, _) -> p) x_res_clauses)
          in
          let new_x_tag =
            lub_tags (lookup_var_tag2 x local_env new_param_env) x_tag_usage
          in
          let new_x = update_id_tag x new_x_tag in
          let res_local_env, res_param_env =
            update_var_tag2 x new_x_tag new_local_env new_param_env
          in
      ...
      | TFun (tvar, body) ->
          ...
          let ( ((_, (new_body_tag, _)) as new_body),
                ...
                new_ctr_tag_map,
                changes ) =
            cf_tag_expr body expected_tag param_env local_env ctr_tag_map
          in
          ( TFun (tvar, new_body),
            new_body_tag,
            new_param_env,
      ...
      | Message bs ->
            ...
            List.fold_right bs ~init:([], param_env, local_env, false)
              ~f:(fun (s, p) (acc_bs, acc_param_env, acc_local_env, acc_changes) ->
                ...
                | MVar x ->
                    ...
                    let new_x = update_id_tag x new_x_tag in
                    let new_local_env, new_param_env =
                      update_var_tag2 x new_x_tag acc_local_env acc_param_env
                    in
                    ( (s, MVar new_x) :: acc_bs,
                      new_param_env,
                      new_local_env,
          ...
          in
          (Message new_bs, NotMoney, new_param_env, new_local_env, ctr_tag_map, changes)
    in
  ...
  let rec cf_tag_stmt (srep : CFSyntax.stmt_annot) param_env field_env local_env
      ctr_tag_map =
    let s, rep = srep in
    let new_s, new_param_env, new_field_env, new_local_env, new_ctr_tag_map, changes =
      ...
      | Bind (x, e) ->
          ...
          let ( ((_, (new_e_tag, _)) as new_e),
                ...
                new_ctr_tag_map,
                e_changes ) =
            cf_tag_expr e x_tag param_env e_local_env ctr_tag_map
          in
      ...
      | MapUpdate (m, ks, v_opt) ->
          let v_tag =
            match v_opt with
            | None -> NoInfo
            | Some v -> lookup_var_tag2 v local_env param_env
          in
          ...
          let new_ks =
            List.map ks ~f:(fun k ->
                let new_k_tag =
                  lub_tags NotMoney (lookup_var_tag2 k local_env param_env)
                in
                update_id_tag k new_k_tag)
          in
          ...
          let new_v_opt, new_param_env, new_local_env =
            ...
            | Some v ->
                let v_tag_usage =
                  List.fold_left ks ~init:m_tag ~f:(fun acc_tag _ ->
                      match acc_tag with Map t -> t | _ -> Inconsistent)
                in
                let new_v_tag = lub_tags v_tag_usage v_tag in
                let new_v = update_id_tag v new_v_tag in
                let new_local_env, new_param_env =
                  update_var_tag2 v new_v_tag ks_local_env ks_param_env
                in
                (Some new_v, new_param_env, new_local_env)
          in
      ...
      | MapGet (x, m, ks, fetch) ->
          ...
          in
          let m_tag_usage = List.fold_left ks ~init:val_tag ~f:(fun acc _ -> Map acc) in
          let m_tag = lub_tags m_tag_usage (lookup_var_tag m field_env) in
          let new_m = update_id_tag m m_tag in
          let new_field_env = update_var_tag m m_tag field_env in
          let new_local_env = AssocDictionary.remove (get_id x) local_env in
          let new_ks =
            List.map ks ~f:(fun k ->
                let new_k_tag =
                  lub_tags NotMoney (lookup_var_tag2 k new_local_env param_env)
                in
                update_id_tag k new_k_tag)
          in
          let ks_local_env, ks_param_env =
            update_vars_tags2 new_ks new_local_env param_env
          in
          let v_tag_usage =
            List.fold_left ks ~init:m_tag ~f:(fun acc_tag _ ->
                match acc_tag with Map t -> t | _ -> Inconsistent)
          ...
          let new_x_tag =
            if fetch then lub_tags x_tag (Adt ("Option", [ new_val_tag ])) else NotMoney
            (* Bool *)
          in
          let new_x = update_id_tag x new_x_tag in
            ...
            List.fold_right clauses
              ...
              ~f:(fun (p, sp)
                ...
                let ( new_stmts,
                      ...
                      s_ctr_tag_map,
                      s_changes ) =
                  cf_tag_stmts sp acc_param_env acc_field_env sub_local_env
                    acc_ctr_tag_map
                in
                let new_p, new_local_env, new_ctr_tag_map, p_changes =
                  update_pattern_vars_tags_from_usage_and_remove_from_env p s_local_env
                    s_ctr_tag_map
                in
            ...
            List.fold_right res_clauses ~init:([], res_clause_changes)
              ~f:(fun (p, sp) (acc_ps, acc_changes) ->
                let new_p, p_change =
                  update_pattern_vars_tags_from_scrutinee p (get_id_tag x)
                in
                ((new_p, sp) :: acc_ps, p_change || acc_changes))
          in
          let x_tag_usage =
            lub_pattern_tags (List.map ~f:(fun (p, _) -> p) res_clauses)
          in
          let new_x_tag =
            lub_tags (lookup_var_tag2 x new_local_env new_param_env) x_tag_usage
          in
          let new_x = update_id_tag x new_x_tag in
          let res_local_env, res_param_env =
            update_var_tag2 x new_x_tag new_local_env new_param_env
      ...
      | SendMsgs m ->
          ...
          let new_m = update_id_tag m m_tag in
          let new_local_env, new_param_env =
            update_var_tag2 m m_tag local_env param_env
          in
          ( SendMsgs new_m,
            new_param_env,
            field_env,
      ...
      | Throw xopt -> (
          ...
          | Some x ->
              ...
              ( Throw (Some new_x),
                ...
                ctr_tag_map,
                get_id_tag x <> x_tag )
          | None -> (Throw None, param_env, field_env, local_env, ctr_tag_map, false) )
    in
  ...
  and cf_tag_stmts ss param_env field_env local_env ctr_tag_map =
    let init_local_env =
      List.fold_left ss ~init:local_env ~f:(fun acc_env srep ->
          let s, _ = srep in
          match s with
    ...
    List.fold_right ss
      ...
      ~f:(fun s
        ...
        let ( new_s,
              ...
              new_ctr_tag_map,
              new_changes ) =
          cf_tag_stmt s acc_param_env acc_field_env acc_local_env acc_ctr_tag_map
        in
        ( new_s :: acc_ss,
          new_param_env,
          new_field_env,
  ...
  let cf_tag_component t param_env field_env ctr_tag_map =
    ...
    let implicit_local_env =
      AssocDictionary.insert MessagePayload.amount_label Money
        (AssocDictionary.insert MessagePayload.sender_label NotMoney empty_local_env)
    in
    let init_local_env =
      List.fold_left comp_params ~init:implicit_local_env ~f:(fun acc_env (p, _) ->
    ...
    let ( new_comp_body,
          ...
          new_ctr_tag_map,
          body_changes ) =
      cf_tag_stmts comp_body param_env field_env init_local_env ctr_tag_map
    in
    ...
    in
    ( { comp_type; comp_name; comp_params = new_params; comp_body = new_comp_body },
      new_param_env,
      new_field_env,
      new_ctr_tag_map,
      new_changes )
  ...
  let cf_tag_contract c =
    ...
    let init_field_env =
      List.fold_left cfields ~init:implicit_field_env ~f:(fun acc_env (f, _, e) ->
          let (_, (e_tag, _)), _, _, _, _ =
            cf_tag_expr e
              (lub_tags (get_id_tag f) NoInfo)
    ...
    let rec tagger cconstraint components param_env field_env ctr_tag_map =
      let new_constraint, tmp_param_env, _, tmp_ctr_tag_map, constraint_changes =
        cf_tag_expr cconstraint NotMoney param_env
          (AssocDictionary.make_dict ())
          ctr_tag_map
      in
      let new_ts, new_param_env, new_field_env, tmp_ctr_tag_map, changes =
        List.fold_right components
          ~init:([], tmp_param_env, field_env, tmp_ctr_tag_map, constraint_changes)
          ~f:(fun t
                  (acc_ts, acc_param_env, acc_field_env, acc_ctr_tag_map, acc_changes)
                  ->
            ...
            ( new_t :: acc_ts,
              ...
              new_field_env,
              new_ctr_tag_map,
              acc_changes || t_changes ))
      in
      if changes then
        tagger new_constraint new_ts new_param_env new_field_env tmp_ctr_tag_map
      else (new_constraint, new_ts, new_param_env, new_field_env, tmp_ctr_tag_map)
    in
    let new_constraint, new_ccomps, new_param_env, new_field_env, final_ctr_tag_map =
      tagger cconstraint ccomps init_param_env init_field_env ctr_tag_map
    in
    let new_params =
      List.fold_right cparams ~init:[] ~f:(fun (p, t) acc_params ->
          let new_tag = lookup_var_tag p new_param_env in
          (update_id_tag p new_tag, t) :: acc_params)
    in
    let new_fields =
      List.fold_right cfields ~init:[] ~f:(fun (f, t, e) acc_fields ->
