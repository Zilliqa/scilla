...
let test1 = test_case (fun _ ->
  ...
  match decode_bech32_addr "zil" bech32_addr with
  | Some b -> assert_bool "Bech32 address decode failed" 
                          (b = decoded_gold && is_valid_bech32 "zil" bech32_addr)
...
let test3 = test_case (fun _ ->
  ...
  | None -> assert_bool "Bech32 address incorrectly validated" (not (is_valid_bech32 "zil" bech32_addr))
)
...
let test4 = test_case (fun _ ->
  ...
  match encode_bech32_addr "zil" input with
...
let random_test seed =
  ...
  in
  match encode_bech32_addr "zil" bystr20_r with
  | Some encoded_str ->
    (match decode_bech32_addr "zil" encoded_str with
