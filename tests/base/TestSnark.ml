...
let test_invalid =
  test_case (fun _ ->
      ...
      in
      let invalid_x, invalid_y =
        ( String.sub x ~pos:0 ~len:3
          ^ (Char.to_int x.[3] lxor 1 |> Char.of_int_exn |> String.of_char)
          ^ String.sub x ~pos:4 ~len:(String.length x - 4),
          y )
      in
      match
        alt_bn128_G1_add { g1x = x; g1y = y }
          { g1x = invalid_x; g1y = invalid_y }
      with
      | Some _ ->
          assert_failure
            "TestSnark failed: test_invalid: alt_bn128_add succeeded on \
             invalid input"
      | None -> ())
...
let test_pairing =
  test_case (fun _ ->
      ...
      let vkx' =
        List.fold2_exn (List.tl_exn vk_IC) input ~init:vkx_init
          ~f:(fun acc a b -> add_helper acc (mul_helper a b))
      in
      let vkx = add_helper vkx' (List.hd_exn vk_IC) in

      (* Now run the pairing checks. *)
      assert_bool
        "TestSnark: test_pairing1: alt_bn128_pairing_product incorrect result"
        (pairingprod_helper [ (proof_A, vk_A); (negateG1 proof_Ap, p2) ]);

      assert_bool
        "TestSnark: test_pairing2: alt_bn128_pairing_product incorrect result"
        (pairingprod_helper [ (vk_B, proof_B); (negateG1 proof_Bp, p2) ]);

      assert_bool
        "TestSnark: test_pairing3: alt_bn128_pairing_product incorrect result"
        (pairingprod_helper [ (proof_C, vk_C); (negateG1 proof_Cp, p2) ]);

      assert_bool
        "TestSnark: test_pairing4: alt_bn128_pairing_product incorrect result"
        (pairingprod_helper
           [
             (proof_K, vk_gamma);
             ( negateG1 (add_helper vkx (add_helper proof_A proof_C)),
               vk_gammaBeta2 );
             (negateG1 vk_gammaBeta1, proof_B);
           ]);

      assert_bool
        "TestSnark: test_pairing5: alt_bn128_pairing_product incorrect result"
        (pairingprod_helper
           [
      ...
      assert_bool
        "TestSnark: test_pairing6: alt_bn128_pairing_product incorrect result"
        (not (pairingprod_helper [ (proof_A, vk_A); (proof_Ap, p2) ])))

let test_pairing_null_input =
  test_case (fun _ ->
