...
let test_invalid =
  test_case (fun _ ->
      ...
      in
      match
        alt_bn128_G1_add { g1x = x; g1y = y } { g1x = invalid_x; g1y = invalid_y }
      with
      | Some _ ->
          assert_failure
            "TestSnark failed: test_invalid: alt_bn128_add succeeded on invalid input"
      | None -> ())
...
let test_pairing =
  test_case (fun _ ->
      ...
      let vk_B =
        {
          g1x =
            hex2bystr "0x2eca0c7238bf16e83e7a1e6c5d49540685ff51380f309842a98561558019fc02";
          g1y =
            hex2bystr "0x03d3260361bb8451de5ff5ecd17f010ff22f5c31cdf184e9020b06fa5997db84";
        }
      in
      let vk_C =
        {
      ...
      let vk_gammaBeta1 =
        {
          g1x =
            hex2bystr "0x15794ab061441e51d01e94640b7e3084a07e02c78cf3103c542bc5b298669f21";
          g1y =
            hex2bystr "0x14db745c6780e9df549864cec19c2daf4531f6ec0c89cc1c7436cc4d8d300c6d";
        }
      in
      let vk_gammaBeta2 =
        {
      ...
      let vkx' =
        List.fold2_exn (List.tl_exn vk_IC) input ~init:vkx_init ~f:(fun acc a b ->
...
let test_pairing_null_input =
  test_case (fun _ ->
      ...
      assert_bool "TestSnark: test_pairing_null_input1: failed" (pairingprod_helper []);
