...
module type TestSuiteMultiInput = sig
  val tests : string list list
  val gold_path : string -> string -> string list
  val test_path : string -> string list
  val runner : string
  val ignore_predef_args : bool
...
module DiffBasedMultiTests (Input : TestSuiteMultiInput) = struct
  open Input
  ...
  let get_basename fnames =
    List.fold_left fnames
      ~init:(List.hd_exn fnames |> String.to_list)
      ~f:(fun acc fname -> longest_common_string acc (String.to_list fname))
    |> String.of_char_list

  let build_exp_tests env =
    let f fnames =
      let open FilePath in
      let basename = get_basename fnames in
      basename >:: fun test_ctxt ->
      let dir = env.tests_dir test_ctxt in
      let input_files =
        List.map fnames ~f:(fun fname -> make_filename (test_path fname))
      in
      let init_file = make_filename (test_path (basename ^ ".json")) in
      (* Verify standard output of execution with gold file *)
      let goldoutput_file = make_filename (gold_path dir basename) in
      let additional_dirs = List.map ~f:make_filename additional_libdirs in
      let stdlib = make_relative dir (env.stdlib_dir test_ctxt) in
      let path = string_of_path @@ (stdlib :: additional_dirs) in
      let args' =
        if ignore_predef_args then custom_args @ input_files
        else
          custom_args
      ...
      assert_command
        ~foutput:(fun s ->
          let actual_output = stream_to_string s in
          if env.update_gold test_ctxt then
            output_updater goldoutput_file basename actual_output
              ~json_errors:false
          else if Sys_unix.file_exists_exn goldoutput_file then
            let gold_output =
              (* load all data from file *)
