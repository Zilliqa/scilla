...
let t5_pn =
  test_case (fun _ ->
      ...
      let p1 = [ m1; m2 ] in
      (* A + B *)
      let sq = mul_pn p1 p1 in
      (* A^2 + 2AB + B^2 *)
      let sq' =
        [ (1, [ ('A', 2) ]); (1, [ ('B', 2) ]); (2, [ ('A', 1); ('B', 1) ]) ]
      in
...
let t6_pn =
  test_case (fun _ ->
      ...
      let b = expand_parameters_pn a ~f in
      let bexpected =
        [ (1, [ ('A', 2) ]); (2, [ ('A', 3) ]); (1, [ ('A', 4) ]) ]
      in
...
let all_tests =
  "polynomial_tests"
  >::: [ t1_pn; t2_pn; t3_pn; t4_pn; t5_pn; t6_pn; t7_pn; t8_pn ]
