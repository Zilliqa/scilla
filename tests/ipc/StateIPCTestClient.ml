...
let initialize ~sock_addr ~(fields : state_info) = stateref := Some (sock_addr, fields)
...
let translate_res res =
  ...
  | Error (e : RPCError.err_t) ->
      assert_failure
        (Printf.sprintf "StateIPCTestClient: Error in IPC access: (code:%d, message:%s)."
           e.code e.message)
  | Ok res' -> res'
...
let fetch ~fname =
  ...
  let tp =
    ...
    | Some tp -> tp
    | None -> assert_failure ("StateIPCTestClient: Unable to find field " ^ fname)
  in
  ...
  | false, _ ->
      assert_failure ("StateIPCTestClient: Field " ^ fname ^ " not found on server")
